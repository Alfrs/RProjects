...
dataset_year_bucket <- dataset %>% filter(!is.na(YEAR_BUCKET))

ggplotly(
ggplot(dataset_year_bucket %>% count(YEAR_BUCKET),aes(x = YEAR_BUCKET, y = n, fill = as.factor(YEAR_BUCKET)))+
  geom_col()+
  geom_text(aes(label = n),position = position_stack(vjust = .5)) +
  ggtitle("Count of Companies by Year Bucket")+
  theme(plot.title = element_text(hjust = 0.5),
...
```{r}
dataset_filtered <- dataset_year_bucket %>% filter(YEAR_INCORP >= 1972)
...
dataset_country_f <- tail(dataset_country_2[order(dataset_country_2$n),])

dataset_country_f$COUNTRY <- factor(dataset_country_f$COUNTRY, levels = dataset_country_f$COUNTRY[order(dataset_country_f$n)])
